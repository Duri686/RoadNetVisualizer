...
export default class AppEventManager {
  ...
  setupPathRequestListener() {
    window.addEventListener('renderer-path-request', (e) => {
      ...
      if (start && end && this.renderer.roadNetData) {
        ...
        const unifiedLayer = this.buildUnifiedGraph(
          this.renderer.roadNetData.layers,
        );
  ...
  buildUnifiedGraph(layers) {
    ...
    const validEdges = allEdges.filter((edge) => {
      ...
      if (!hasFrom || !hasTo) {
        invalidEdges++;
        console.warn(
          `[UnifiedGraph] Invalid edge: ${edge.from} -> ${edge.to} (from exists: ${hasFrom}, to exists: ${hasTo})`,
        );
        return false;
      }
      return true;
    ...
    console.log(
      `[UnifiedGraph] Built graph: ${allNodes.length} nodes, ${validEdges.length} edges (${invalidEdges} invalid edges filtered)`,
    );
    console.log(
      `[UnifiedGraph] Cross-floor edges: ${
        validEdges.filter((e) => e.crossFloor).length
      }`,
    );
  ...
  handlePathRequest(layer, start, end) {
    ...
    if (path && path.length > 0) {
      ...
      for (let i = 0; i < path.length; i++) {
        ...
        const isConnector =
          node.id &&
          (String(node.id).includes('C') || String(node.id).includes('access'));
        const style = isConnector
          ? 'color: orange; font-weight: bold'
          : 'color: #888';
        console.log(
          `%c[${i}] ${node.id} (${Math.round(node.x)}, ${Math.round(
            node.y,
          )}) L${currentLayer}`,
          style,
        );
        ...
        if (i > 0) {
          ...
          if (currentLayer !== prevLayer) {
            transitions++;
            console.log(
              `%c⚡ Layer Transition: L${prevLayer} -> L${currentLayer}`,
              'color: yellow; background: #333; padding: 2px',
            );
          }
        }
      }
    ...
    if (path && path.length > 1) {
      ...
      try {
        ...
        smoothedPath.forEach((node, i) => {
          ...
          console.log(
            `[${i}] ${node.id} (${Math.round(node.x)}, ${Math.round(
              node.y,
            )}) L${node.layer} (was [${originalIndex}] in original)`,
          );
        });
        console.groupEnd();
  ...
  updatePathStatsUI(path, stats, smoothTime) {
    ...
    if (pathLength)
      pathLength.textContent = `${stats.totalLength.toFixed(2)} m`;
